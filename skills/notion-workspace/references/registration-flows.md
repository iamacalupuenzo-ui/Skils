...
## Flujo 1 — Registrar un Skill

**BD destino:** Skills & Marcos (`collection://0f912612-f1e0-4b3e-9fee-85bd790359c9`)
**Cuándo:** "registra el skill X", "agrega este skill a Notion", "guarda /nombre-skill"

### Paso 0 — Verificar que el skill existe en disco (obligatorio)

Antes de crear la fila, comprueba que la carpeta exista en el repositorio canónico:

```powershell
Test-Path "$env:USERPROFILE\.agents\skills\<nombre-skill>"
```

`~/.agents/skills` es el repositorio real; `~/.claude/skills` es un junction hacia él, así que
da igual cuál mires. Si la carpeta no existe, **no registres el skill** — pregunta si se va a
crear o si el nombre está mal. Registrar skills que no existen fue lo que dejó `/find-skills`
y `/mobile-app-ui-design` colgados en la BD después de borrarlos del repositorio.

Verifica también que no exista ya una fila con ese nombre (B2).
...
### Cierre
```
Skill registrado — [/nombre]
  BD:          Skills & Marcos (Process OS)
  URL:         [URL]
  Cuándo usar: [texto]
  Fase / Rol:  [valores]
  Pasos:       [lista o "ninguno"]
```
...
**Cuándo:** "registrá el proyecto X", "creá el proyecto de Y", "nuevo proyecto"
...
**Cuándo:** "registrá la herramienta X", "agregá Figma/Slack/X a Notion", "nueva herramienta"
...
**Cuándo:** "registrá el objetivo de X", "creá una meta", "nuevo goal", "quiero lograr X"
...
**Cuándo:** "guardá esta nota", "anotá esto", "registrá esta idea", "capturá esto en LifeOS"
...
**Cuándo:** "creá la plantilla de X", "agregá este entregable", "necesito un formato para",
"documentá la estructura de", "qué preguntas van en el brief de"
...
| "nota", "anotá", "capturá", "recordá" | Nota |
...
3. Si el proyecto no existe, registrarlo primero por el Flujo 3.
4. Registrar el responsable como texto, respetando el nombre y las asignaciones compartidas de la fuente.
5. Si hay que descomponer en subtareas, aplicar primero "Validación de secuencia al desglosar
   en subtareas" del `SKILL.md` — separar pasos compuestos, detectar dependencias obligatorias
   y fijar `N° orden` + `Depende de` antes de crear las filas. No crear la secuencia en el
   mismo orden narrativo sin antes verificar que cada paso es ejecutable con el anterior ya cerrado.
...
> ¿Quieres una tarea única o subtareas? Si son subtareas, se crean como filas de `Tareas`
> enlazadas por `Tarea padre`/`Subtareas` (self-relación en la misma BD) — no existe un
> contenedor de tipo Épica o User Story en este espacio.
