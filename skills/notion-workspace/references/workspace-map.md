...
4 ambientes principales. Cada uno es un hub page que contiene una o más BDs.
Este archivo es la fuente de verdad para routing de operaciones.
...
**Tareas — campos de gestión:** `Nombre`, `Fase de lanzamiento`, `Estado`, `Completada`, `Progreso`, `Avance de subtareas (%)`, `Prioridad`, `Responsable`, `Ejecutor`, `Fecha compromiso`, `Fecha real`, `Proyecto`, `Entregable`, `Descripción`, `Criterio de cierre`, `Tarea padre`, `Subtareas`, `Depende de`, `Bloquea a` y `Referencias`.
...
- Las anotaciones, cantidades, dependencias y criterios incluidos con la actividad van en `Descripción`.
- **No existe ningún campo numérico manual de avance.** `Progreso` es una fórmula de solo lectura que calcula el % automáticamente: si la tarea tiene `Subtareas`, promedia cuántas de ellas tienen `Completada` marcada (vía el rollup `Avance de subtareas (%)`); si no tiene subtareas, usa 100 si `Estado = Hecho` y 0 en cualquier otro caso. Se muestra como barra de bloques + porcentaje, coloreada por rango (0-24 rojo, 25-49 naranja, 50-74 amarillo, 75-99 azul, 100 verde). Nunca escribir un número directamente en `Progreso` ni en `Avance de subtareas (%)` — para mover el avance de una tarea sin subtareas, cambiar su `Estado`; para una tarea con subtareas, cerrar (`Completada`) las subtareas correspondientes.
- Las subtareas son filas reales de esta misma BD conectadas por `Tarea padre` (en la hija) y `Subtareas` (en la padre) — **la relación no se sincroniza sola en los dos sentidos**, hay que escribir ambos lados explícitamente tras crear las filas. No existe un contenedor de tipo Épica o User Story en este espacio.
- Antes de crear las subtareas, aplicar "Validación de secuencia al desglosar en subtareas"
  del `SKILL.md`: separar pasos compuestos que escondan un prerrequisito, fijar `N° orden` y
  enlazar `Depende de`/`Bloquea a` cuando exista una dependencia obligatoria real entre pasos —
  no asumir que el orden narrado por el usuario ya es el orden de ejecución correcto.
...
**Sincronización con el disco:** la fuente de verdad de los skills es `~/.agents/skills`
(repositorio git `claude-skills`); `~/.claude/skills` es un junction hacia él. Antes de dar de
alta un skill, verificar que la carpeta exista. Al eliminar un skill del repositorio, archivar
su fila con `notion-move-pages` hacia la página de archivo.
...
| Un skill de Claude | Process OS | Skills & Marcos | `0f912612-f1e0-4b3e-9fee-85bd790359c9` |
