...
const COMMENT  = figma.root.findOne(n => n.name === 'Comment' &&
...
```javascript
// 1. Resolver un componente concreto y crear una instancia.
const MAIN = COMMENT.type === 'COMPONENT_SET'
  ? COMMENT.children.find(n => n.type === 'COMPONENT')
  : COMMENT;
if (!MAIN) throw new Error('Comment no contiene una variante COMPONENT utilizable');
const obs = MAIN.createInstance();
...
Crear una instancia desde un componente concreto permite usar componentProperties y setProperties.
Si hay varias variantes, seleccionar la que corresponda por propiedades verificadas, no la primera.
