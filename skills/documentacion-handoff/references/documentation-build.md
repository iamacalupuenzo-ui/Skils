...
**Idempotencia conservadora:** no borrar por nombre. Si existe una versión, elegir según el
pedido: actualizarla preservando su identidad, crear una versión nueva claramente nombrada,
o detenerse para decidir. Retirar la anterior requiere autorización específica y verificación
de instancias, enlaces e histórico.

```javascript
const previas = page.children.filter(c => c.name === '<Componente> — Documentación');
return previas.map(c => ({ id: c.id, name: c.name, type: c.type }));
// No llamar remove() en el flujo de construcción. Si se autorizó retirar una versión,
// hacerlo en una operación separada después de volver a identificarla por id.
```
